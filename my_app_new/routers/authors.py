...
def create_author(author: schemas.AuthorCreate, db: Session = Depends(get_async_session)):
    db_author = crud.get_author_by_author_name(db, author_name=author.author_name)
    if db_author:
...
def read_authors(skip: int = 0, limit: int = 100, db: Session = Depends(get_async_session)):
